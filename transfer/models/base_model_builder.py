...
def build_base_thin_model():
    input_shape, train_data, train_labels, test_data, test_labels = get_mnist_data()
    model = tf.keras.models.Sequential()
    model.add(tf.keras.layers.Conv2D(8, kernel_size=(3, 3), activation='relu', input_shape=input_shape, name='first'))
    model.add(tf.keras.layers.MaxPooling2D(pool_size=(2, 2)))
    ...
    history = model.fit(x=train_data, y=train_labels, batch_size=128, epochs=30, verbose=1)
